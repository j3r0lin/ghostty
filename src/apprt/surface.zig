...
pub const Message = union(enum) {
    ...
    pub const DesktopNotification = struct {
        ...
        pub fn init(
            ...
            copyUtf8Z(result.agent.len, &result.agent, agent);
            copyUtf8Z(result.state.len, &result.state, state);
            return result;
        }
...
test "DesktopNotification init" {
    const notification = Message.DesktopNotification.init("Title", "Body", "claude", "done");
    ...
    try std.testing.expectEqualStrings("claude", std.mem.sliceTo(&notification.agent, 0));
    try std.testing.expectEqualStrings("done", std.mem.sliceTo(&notification.state, 0));
}
